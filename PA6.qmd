...
In our data cleaning process we will omit observations with at least one variable missing. We are aware that omitting these missing variables decreases the sample size and may lead us to miss important information. Since we are using ggplot and lm--which drop NA obs anyway--it makes sense to remove these variables for the entire analysis.
...
```{r}
co2_cdmt %>%
  filter(country == "Estonia" | country == "Latvia" | country == "Latvia") %>%
  mutate(year = as.numeric(year)) %>%
  filter(year %% 10 == 0) %>%
  ggplot(aes(x = year, y = cdmt, size = co2)) +
  geom_point(alpha = 0.7)
```

```{r}
co2_cdmt %>%
  filter(country %in% c("Estonia", "Latvia")) %>%
  mutate(cdmt = as.numeric(cdmt)) %>%
  mutate(co2 = as.numeric(co2)) %>%
  mutate(year = as.numeric(year),
         log_cdmt = log(cdmt + 1)) %>%  # Adding 1 to avoid taking log of zero
  filter(year %% 10 == 0) %>%
  ...
  scale_color_manual(values = c("Estonia" = "blue", "Latvia" = "red")) +
  theme_minimal()
```
