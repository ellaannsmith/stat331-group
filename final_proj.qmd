...
```
```{r}
co2_cdmt %>%  mutate(year = as.numeric(year),         co2 = as.numeric(co2),         cdmt = as.numeric(cdmt)) %>%  group_by(year) %>%  summarize(avg_co2 = mean(co2),            avg_cdmt = mean(cdmt)) %>%  ggplot(aes(x = avg_co2, y = avg_cdmt)) +  geom_jitter(alpha = 0.7, color = "steelblue", size = 3) +    labs(title = "Average CO2 vs. CDMT",       subtitle = "Average CDMT",       x = "Average CO2",       y = "") +    theme_minimal() +  theme(plot.title = element_text(size = 20, face = "bold"),        plot.subtitle = element_text(size = 12),        axis.text = element_text(size = 10))
