...
There appears to be a negative relationship between the logged average values of co2 and child mortality. There appears to be outliers on the low end and a slight fanning of the points as co2 increases. There does seem to be a moderate linear correlation between the co2 and cdmt.
...
There is no distinct pattern between the residuals and the predicted values. Since there is no clear fanning we can assume the equal variance is met. Similarly since the points are evenly distributed about the line, we can also assume linearity is met.
...
The observed data is more skewed than the similar data. Normality does not appear to be met.
...
obs_reg_p <- graph |>
  ggplot(aes(y = log_avg_cdmt,
             x = log_avg_co2)) +
  geom_point() +
  labs(title = "Observed CDMT",
       x = "log_avg_co2",
       y = "",
       subtitle = "log_avg_co2") +
  theme_bw()

sim_reg_p <-sim_data |>
  ggplot(aes(y = sim_cdmt,
             x = log_avg_co2)
         ) +
  geom_point() +
   labs(title = "Simulated CDMT ",
        x = "log_avg_co2",
       y = "",
       subtitle = "log_avg_co2") +
  theme_bw()
...
#| code-fold: true
sim_data |> 
  ggplot(aes(x = sim_cdmt, 
             y = log_avg_co2)
         ) + 
  geom_point() + 
   labs(x = "Simulated CDMT", 
        y = "",
        subtitle = "Observed CDMT" ) + 
  geom_abline(slope = 1,
              intercept = 0, 
              color = "red",
              linetype = "dashed",
